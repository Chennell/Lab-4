...
//@ $db = new mysqli('localhost', 'root', '1234', 'Book');
...
if (isset($_POST['username'], $_POST['userpass'])) {
    ...
    //echo "SELECT * FROM testinguser WHERE username = '{$uname}' AND userpassword = '{$upass}'";
    
    $query = ("SELECT * FROM testinguser WHERE username = '{$uname}' "."AND userpassword = '{$upass}'");
    ...
    <body>
        ...
        if (isset($totalcount)) {
            if ($totalcount == 0) {
                ...
                echo '<a href=fileUpload.php> Here is the link to upload </a>';
            }
        }
        ?>
        <form method="POST" action="">
           <h2>Login to upload pictures</h2>
            <td>Username:</td>
            <input type="text" name="username">
            <td>Password:</td>
            <input type="password" name="userpass">
            <input type="submit" value="Login">
            <div class="endOfLogin"></div>
        </form>
